...
export default function ClinicSettingsPage() {
    ...
    const LabeledInput = ({
        icon, label, field, placeholder, textarea = false
    }: {
    ...
    return (
        <div className="flex flex-col h-full overflow-y-auto">
            ...
            <div className="px-4 py-5 space-y-5 pb-12">
                ...
                <LabeledInput icon={<Building2 size={16} />} label="Tên phòng mạch" field="name" placeholder="Phòng khám Minh Tuệ" />
                <LabeledInput icon={<Stethoscope size={16} />} label="Chuyên về" field="specialty" placeholder="Nội tiết, Chuyển hóa, Dinh dưỡng" />
                <LabeledInput icon={<Hash size={14} />} label="Mô tả" field="description" placeholder="Phòng khám chuyên sâu về y học chuyển hóa..." textarea />
                <LabeledInput icon={<Clock size={16} />} label="Giờ hoạt động" field="working_hours" placeholder="Thứ 2 - Thứ 7: 8:00 - 17:30" />

                {/* Section: Kinh doanh */}
                <p className="text-[11px] text-gray-500 uppercase tracking-widest pt-2">Thông tin kinh doanh</p>
                <LabeledInput icon={<User size={16} />} label="Bác sĩ phụ trách" field="doctor_name" placeholder="BS. Nguyễn Minh Tuệ" />
                <LabeledInput icon={<Hash size={14} />} label="Mã số thuế" field="tax_id" placeholder="0123456789" />

                {/* Section: Địa điểm */}
                <p className="text-[11px] text-gray-500 uppercase tracking-widest pt-2">Địa điểm</p>
                <LabeledInput icon={<MapPin size={16} />} label="Địa chỉ" field="address" placeholder="123 Nguyễn Trãi, Quận 5, TP.HCM" />
                <LabeledInput icon={<MapPin size={16} />} label="Google Maps Embed URL" field="map_url" placeholder="https://maps.google.com/maps?q=..." />
